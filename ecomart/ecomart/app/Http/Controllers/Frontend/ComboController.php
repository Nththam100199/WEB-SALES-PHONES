...
{   
    // ajax 
    public function postSortCuaHang(){

        $value = $_POST['value'];

        if($value == "noibat"){

            $product_list = Product::OrderBy('hot','DESC')->get();

        }
        if($value == "combo"){

            $product_list = Product::OrderBy('combo','DESC')->get();

        }
        if($value == "khuyenmai"){

            $product_list = Product::where('sale','!=',0)->OrderBy('combo','DESC')->get();

        }

        return view('frontend.combo.sort-cuahang-ajax',compact('product_list'));
    }
    // endajax
